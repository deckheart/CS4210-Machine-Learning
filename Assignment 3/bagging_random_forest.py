...
# SPECIFICATION: Evaluate accuracies of bagging and random forest classification on a data set
# FOR: CS 4210- Assignment #3
# TIME SPENT: 2 hours for bagging_random_forest.py
#-----------------------------------------------------------*/
...
with open(optdigits_tes, 'r') as testingFile:
    ...
    for k in range(20): #we will create 20 bootstrap samples here (k = 20). One classifier will be created for each bootstrap sample
        ...
        for i, testSample in enumerate(dbTest):
            ...
            ts = testSample[:-1]
            predict = clf.predict([ts])[0]
            classVotes[i][int(predict)] += 1

            if k == 0: #for only the first base classifier, compare the prediction with the true label of the test sample here to start calculating its accuracy
                if predict == testSample[-1]:
                    accuracy += 1
    ...
    #and compare the Random Forest prediction for each test sample with the ground truth label to calculate its accuracy
    accuracy = 0
    for i, row in enumerate(dbTest):
        train_row = row[:-1]
        predict = clf.predict([train_row])
        if int(predict) == int(row[-1]):
            accuracy += 1
    accuracy = accuracy/len(dbTest)
